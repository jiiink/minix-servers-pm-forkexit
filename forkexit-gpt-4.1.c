...
int do_fork(void)
{
    ...
    if (n > NR_PROCS || next_child >= NR_PROCS || (mproc[next_child].mp_flags & IN_USE))
        panic("do_fork can't find valid child slot: %d", next_child);
...
{
    if (mp == NULL) {
        fprintf(stderr, "Error: mp is NULL in do_exit\n");
        return SUSPEND;
    }

    if (mp->mp_flags & PRIV_PROC) {
        printf("PM: system process %d (%s) tries to exit(), sending SIGKILL\n",
               mp->mp_endpoint, mp->mp_name ? mp->mp_name : "unknown");
        if (sys_kill(mp->mp_endpoint, SIGKILL) != 0) {
            fprintf(stderr, "Error: sys_kill failed for endpoint %d\n", mp->mp_endpoint);
        }
    } else {
...
void exit_proc(struct mproc *rmp, int exit_status, int dump_core)
{
    ...
    for (struct mproc *p = &mproc[0]; p < &mproc[NR_PROCS]; p++) {
        if (!(p->mp_flags & IN_USE))
            continue;
        if (p->mp_tracer == proc_nr)
            tracer_died(p);
        if (p->mp_parent == proc_nr) {
            p->mp_parent = INIT_PROC_NR;
            if (p->mp_flags & VFS_CALL)
                p->mp_flags |= NEW_PARENT;
            if (p->mp_flags & ZOMBIE)
                check_parent(p, TRUE);
        }
    }
...
int wait_test(struct mproc *rmp, struct mproc *child)
{
    ...
    if (pidarg == -1 || pidarg == child->mp_pid || -pidarg == child->mp_procgrp)
        return 1;
...
static void check_parent(struct mproc *child, int try_cleanup)
{
    ...
    if (!child || child->mp_parent < 0 || child->mp_parent >= NR_PROCS)
        return;
...
static int tell_parent(struct mproc *child, vir_bytes addr)
{
    ...
    if (addr != 0) {
        ...
        r = sys_datacopy(SELF, (vir_bytes)&r_usage, parent->mp_endpoint, addr, sizeof(r_usage));
        if (r != OK) {
...
{
    if (!child)
        panic("tell_tracer: null child pointer");
    ...
    if (child->mp_tracer >= NR_PROCS)
        panic("tell_tracer: tracer index out of bounds: %d", child->mp_tracer);
