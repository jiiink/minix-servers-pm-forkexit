...
do_srv_fork(void)
{
  ...
  } while (mproc[next_child].mp_flags & IN_USE);
  ...
  *rmc = *rmp;
  ...
  if ((rmc->mp_trace_flags & TO_TRACEFORK) == 0) {
    rmc->mp_tracer = NO_TRACER;
    rmc->mp_trace_flags = 0;
...
)
{
  ...
  if (dump_core) {
    if (rmp->mp_realuid != rmp->mp_effuid || (rmp->mp_flags & PRIV_PROC)) {
      dump_core = FALSE;
    }
  }
...
int do_wait4(void)
{
    ...
    for (rp = &mproc[0]; rp < &mproc[NR_PROCS]; rp++) {
        if (!is_valid_child(rp, who_p, pidarg)) {
            continue;
        }
        ...
        if (rp->mp_tracer == who_p) {
            int result = handle_traced_child(rp, addr);
            if (result != 0) {
                return result;
            }
        }
...
static int is_valid_child(struct mproc *rp, int parent, int pidarg)
{
    if ((rp->mp_flags & (IN_USE | TOLD_PARENT)) != IN_USE) {
        return 0;
    }
    
    if (rp->mp_parent != parent && rp->mp_tracer != parent) {
        return 0;
    }
    
    if (rp->mp_parent != parent && (rp->mp_flags & ZOMBIE)) {
        return 0;
    }
...
static int handle_traced_child(struct mproc *rp, vir_bytes addr)
{
    int i;
...
int wait_test(struct mproc *rmp, struct mproc *child)
{
    ...
    if (pidarg < 0 && -pidarg == child->mp_procgrp) {
        return 1;
    }
...
zombify(struct mproc *rmp)
{
    ...
    has_separate_tracer = (rmp->mp_tracer != NO_TRACER) && 
                          (rmp->mp_tracer != rmp->mp_parent);
    
    if (has_separate_tracer) {
        rmp->mp_flags |= TRACE_ZOMBIE;
        
        t_mp = &mproc[rmp->mp_tracer];
        
        if (wait_test(t_mp, rmp) != 0) {
            tell_tracer(rmp);
        }
    } else {
        rmp->mp_flags |= ZOMBIE;
...
static void check_parent(struct mproc *child, int try_cleanup)
{
    ...
    if (wait_test(parent, child)) {
        handle_waiting_parent(child, parent, try_cleanup);
    } else {
        notify_parent_with_signal(parent);
    }
}

static void handle_waiting_parent(struct mproc *child, struct mproc *parent, int try_cleanup)
{
    int cleanup_needed = tell_parent(child, parent->mp_waddr);
...
static void notify_parent_with_signal(struct mproc *parent)
{
...
tell_tracer(struct mproc *child)
{
  ...
  if (child == NULL) {
    panic("tell_tracer: NULL child process");
  }

  if (child->mp_tracer <= 0) {
    panic("tell_tracer: bad value in mp_tracer: %d", child->mp_tracer);
  }
  ...
  tracer = &mproc[child->mp_tracer];
  if (tracer == NULL) {
    panic("tell_tracer: invalid tracer");
  }
